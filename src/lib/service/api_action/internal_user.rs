...
pub async fn web_internal_user_login<'a>(
    req: UserLogin,
    database: &Database,
    app_config: &AppConfig,
    cache: &Cache,
) -> Result<(UserLoginResponse, Cookie<'a>)> {
    // check if device_id is already exist and uuse same session_id instead of create new one
    // turn req: UserLogin to domain for validation ??
    let get_from_db = query::get_internal_user_credentials(database, req.email.trim())
        .await?
        .rows;
    ...
    let claims = Claims::new(employee_id, &session_id, role, jwt_valid_until)?;
...
pub async fn web_internal_refresh_token(
    ...
    let gg = utils::refresh_token::web_refresh_token_helper(
        req_info, req, database, app_config, cache, true,
    )
    .await?;
    Ok(())
}
